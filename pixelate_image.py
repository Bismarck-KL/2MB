...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        input_path = input("Enter image path (or press Enter for 'sample/tpose.png'): ").strip()
        if not input_path:
    ...
    palette_input = input("Enter color palette size (4-32, default 16): ").strip()
    palette_size = int(palette_input) if palette_input.isdigit() else 16
    palette_size = max(4, min(32, palette_size))
    
    # Generate output path
    input_file = Path(input_path)
    output_path = input_file.parent / f"{input_file.stem}_pixel{pixel_size}.png"
    
    print(f"\nProcessing...")
    pixelated = pixelate_image(input_path, str(output_path), pixel_size, palette_size)
