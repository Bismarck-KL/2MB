...
def run_loading_with_callback(
    ...
    *,
    ...
    """Run a loader callable in a background thread while showing the loading UI.

    The loader should accept a `report(progress_float)` argument and may also
    accept an optional `stop_event` threading.Event as a second argument.
    """
    ...
    def report(p: float):
        try:
            progress["value"] = float(max(0.0, min(100.0, p)))
        except Exception:
            progress["value"] = 0.0
    ...
    running = True
    while running and not done_event.is_set():
    ...
    if exc_holder.get("exc") is not None:
        if on_error:
            try:
                on_error(exc_holder["exc"])
            except Exception:
                pass
        else:
            import traceback

            traceback.print_exception(type(exc_holder["exc"]), exc_holder["exc"], exc_holder["exc"].__traceback__)
    else:
        if on_complete and running:
            try:
                on_complete()
            except Exception:
                pass
...
__all__ = ["draw_loading_bar", "draw_loading_screen", "run_loading_with_callback"]
